import { supabase, signInWithGoogle, signOut, onAuthStateChange, currentSession } from "./supabase-client.js";
import { getCredits, generate, startCheckout } from "./api.js";
...
function renderPricing() {
  ...
  el("#upgrade-btn")?.addEventListener("click", async (e) => {
    const btn = e.currentTarget;
    btn.disabled = true;
    btn.innerHTML = `<span class="spin-loader"></span> Redirecting…`;
    try {
      await startCheckout();
    } catch (err) {
      toast(String(err.message || err).replace(/^Bad state: /, ""));
      btn.disabled = false;
