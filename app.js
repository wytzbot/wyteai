...
// ---------------------------------------------------------------------------
async function boot() {
  // Render the public UI immediately. Authentication must never be able to
  // leave the app looking like a blank/black page if Firebase is unavailable.
  render();
  ...
  try {
    ...
    console.warn("Wyte AI: authentication bootstrap failed; keeping welcome screen visible.", e);
    state.session = null;
    render();
  }
  ...
  try {
    ...
    console.warn("Wyte AI: auth state listener failed.", e);
  }
}
